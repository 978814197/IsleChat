...
def route_after_agent_profile_extraction(state: AgentState) -> str:
    """Agent 配置提取后的路由函数。

    在 should_extract_agent_profile 节点执行完毕后，根据提取结果
    决定下一步流向：
    - 如果需要提取且成功提取了 Agent 配置 → 进入 save_agent_profile 节点保存
    - 否则 → 直接结束，跳过保存步骤

    :param state: 当前 Agent 状态。
    :return: 下一个节点的名称，"save_agent_profile" 或 END。
    """
